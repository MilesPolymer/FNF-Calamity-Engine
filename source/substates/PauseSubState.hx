...
import config.OptionsMenu;
import config.Controls.Control;
import flixel.FlxG;
...
{
	var grpMenuShit:FlxTypedGroup<ui.Alphabet>;

	var pauseOG:Array<String> = ['Resume', 'Restart Song', 'Change Difficulty', 'Toggle Practice Mode', 'Options', 'Chart Editor', 'Exit to menu'];
	var difficultyChoices:Array<String> = ['EASY', 'NORMAL', 'HARD', 'BACK'];

	var menuItems:Array<String> = [];
	var curSelected:Int = 0;

	var scoreWarning:FlxText = new FlxText(20, 15 + 64, 0, "WARNING: Changing your options will invalidate your score!", 32);
	...
	public function new(x:Float, y:Float)
	{
		...
		levelDifficulty.text += game.CoolUtil.difficultyString();
		...
		deathCounter.text = 'Blueballed: ' + states.PlayState.deathCounter;
		...
		practiceText.visible = states.PlayState.practiceMode;
		add(practiceText);

		scoreWarning.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, RIGHT);
		...
		grpMenuShit = new FlxTypedGroup<ui.Alphabet>();
		add(grpMenuShit);
	...
	private function regenMenu()
	{
		...
		{
			var menuItem:ui.Alphabet = new ui.Alphabet(0, (70 * i) + 30, menuItems[i], true, false);
		...
		if (accepted)
		{
			...
			switch (daSelected)
			{
				...
				case "Change Difficulty":
					...
					regenMenu();
				case "Options":
					FlxG.switchState(new config.OptionsState());
				case "Chart Editor":
				...
				case "EASY" | "NORMAL" | "HARD":
					states.PlayState.SONG = game.Song.loadFromJson(utilities.Highscore.formatSong(states.PlayState.SONG.song.toLowerCase(), curSelected), states.PlayState.SONG.song.toLowerCase());
