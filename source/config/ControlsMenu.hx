...
class ControlsMenu extends Page
{
	...
	override public function new()
	{
		...
		if (FlxG.gamepads.numActiveGamepads > 0)
		{
			...
			spr.makeGraphic(FlxG.width, 100, 0xFF6B6B6B);
			add(spr);
		...
		prompt.create();
		prompt.createBgFromMargin(100, 0xFF6B6B6B);
	...
	override function update(elapsed:Float)
	{
		...
		var controls = config.PlayerSettings.player1.controls;
	...
	function onInputSelect(input:Int)
	{
		...
		if (controlGrid.members[index].input != input && controlGrid.members[index + 1].input != input)
		{
			for (group in itemGroups)
			{
				if (group.contains(selected))
				{
					for (item in group)
					{
						...
						{
							config.PlayerSettings.player1.controls.replaceBinding(item.control, currentDevice, selected.input, item.input);
			...
			config.PlayerSettings.player1.controls.replaceBinding(selected.control, currentDevice, input, selected.input);
			selected.input = input;
			selected.label.text = selected.getLabel(input);
			config.PlayerSettings.player1.saveControls();
		}
	}
