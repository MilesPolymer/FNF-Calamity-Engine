...
class ChartingState extends MusicBeatState
{
	...
	var curRenderedNotes:FlxTypedGroup<game.Note>;
	...
	**/
	...
	var player2:game.Character = new game.Character(0,0, "dad");
	var player1:game.Boyfriend = new game.Boyfriend(0,0, "bf");

	var leftIcon:ui.HealthIcon;
	var rightIcon:ui.HealthIcon;

	private var lastNote:game.Note;
	var claps:Array<game.Note> = [];
	...
	override function create()
	{
		...
		curRenderedNotes = new FlxTypedGroup<game.Note>();
		...
		loadSong(_song.song);
		game.Conductor.changeBPM(_song.bpm);
		game.Conductor.mapBPMChanges(_song);

		leftIcon = new ui.HealthIcon(_song.player1);
		rightIcon = new ui.HealthIcon(_song.player2);
	...
	function addSongUI():Void
	{
		...
		stepperBPM.value = game.Conductor.bpm;
		...
		var characters:Array<String> = game.CoolUtil.coolTextFile(Paths.txt('characterList'));
		var stages:Array<String> = game.CoolUtil.coolTextFile(Paths.txt('stages'));
		...
		tab_group_assets.name = "Utilities";
	...
	function addSectionUI():Void
	{
		...
		stepperSectionBPM = new FlxUINumericStepper(10, 80, 1, game.Conductor.bpm, 0, 999, 0);
		stepperSectionBPM.value = game.Conductor.bpm;
	...
	function addNoteUI():Void
	{
		...
		stepperSusLength = new FlxUINumericStepper(10, 10, game.Conductor.stepCrochet / 2, 0, 0, game.Conductor.stepCrochet * _song.notes[curSection].lengthInSteps * 4);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'song_bpm')
			{
				...
				game.Conductor.mapBPMChanges(_song);
				game.Conductor.changeBPM(Std.int(nums.value));
			}
	...
	override function update(elapsed:Float)
	{
		...
		var delete = false;
		curRenderedNotes.forEach(function(note:game.Note)
			{
		...
		for (p in 0...pressArray.length)
		{
			...
			if (i && !delete)
			{
				addNote(new game.Note(game.Conductor.songPosition,p));
			}
		}

		strumLine.y = getYfromStrum((game.Conductor.songPosition - sectionStartTime()) % (game.Conductor.stepCrochet * _song.notes[curSection].lengthInSteps));
		...
		{
			changeNoteSustain(game.Conductor.stepCrochet);
		}
		...
		if (!typingShit.hasFocus)
		{
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				var stepMs = curStep * game.Conductor.stepCrochet;


				trace(game.Conductor.stepCrochet / snap);

				if (doSnapShit)
					FlxG.sound.music.time = stepMs - (FlxG.mouse.wheel * game.Conductor.stepCrochet / snap);
				else
					FlxG.sound.music.time -= (FlxG.mouse.wheel * game.Conductor.stepCrochet * 0.4);
				trace(stepMs + " + " + game.Conductor.stepCrochet / snap + " -> " + FlxG.sound.music.time);
			...
			else
			{
				if (FlxG.keys.justPressed.W || FlxG.keys.justPressed.S)
				{
					...
					var daTime:Float = game.Conductor.stepCrochet * 2;
		...
		bpmTxt.text = bpmTxt.text = Std.string(FlxMath.roundDecimal(game.Conductor.songPosition / 1000, 2))
			+ " / "
	...
	function recalculateSteps():Int
	{
		...
		{
			if (FlxG.sound.music.time > game.Conductor.bpmChangeMap[i].songTime)
				lastChange = game.Conductor.bpmChangeMap[i];
		}

		curStep = lastChange.stepTime + Math.floor((FlxG.sound.music.time - lastChange.songTime) / game.Conductor.stepCrochet);
		updateBeat();
	...
	function copySection(?sectionNum:Int = 1)
	{
		...
		{
			var strum = note[0] + game.Conductor.stepCrochet * (_song.notes[daSec].lengthInSteps * sectionNum);
	...
	function updateGrid():Void
	{
		...
		{
			game.Conductor.changeBPM(_song.notes[curSection].bpm);
		...
		else
		{
			...
			for (i in 0...curSection)
				if (_song.notes[i].changeBPM)
					daBPM = _song.notes[i].bpm;
			game.Conductor.changeBPM(daBPM);
		}
		...
		for (i in sectionInfo)
		{
			...
			var note:game.Note = new game.Note(daStrumTime, daNoteInfo % 4);
			note.sustainLength = daSus;
			note.setGraphicSize(GRID_SIZE, GRID_SIZE);
			note.updateHitbox();
			note.x = Math.floor(daNoteInfo * GRID_SIZE);
			note.y = Math.floor(getYfromStrum((daStrumTime - sectionStartTime()) % (game.Conductor.stepCrochet * _song.notes[curSection].lengthInSteps)));
			...
			if (daSus > 0)
			{
				var sustainVis:FlxSprite = new FlxSprite(note.x + (GRID_SIZE / 2),
					note.y + GRID_SIZE).makeGraphic(8, Math.floor(FlxMath.remapToRange(daSus, 0, game.Conductor.stepCrochet * _song.notes[curSection].lengthInSteps, 0, gridBG.height)));
	...
	function selectNote(note:game.Note):Void
	{
		var swagNum:Int = 0;
	...
	function deleteNote(note:game.Note):Void
		{
			lastNote = note;
			for (i in _song.notes[curSection].sectionNotes)
			{
	...
	private function addNote(?n:game.Note):Void
	{
	...
	{
		return FlxMath.remapToRange(yPos, gridBG.y, gridBG.y + gridBG.height, 0, 16 * game.Conductor.stepCrochet);
	}

	function getYfromStrum(strumTime:Float):Float
	{
		return FlxMath.remapToRange(strumTime, 0, 16 * game.Conductor.stepCrochet, gridBG.y, gridBG.y + gridBG.height);
	}
	...
	{
		PlayState.SONG = game.Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
		LoadingState.loadAndSwitchState(new ChartingState());
	}

	function loadAutosave():Void
	{
		PlayState.SONG = game.Song.parseJSONshit(FlxG.save.data.autosave);
		LoadingState.loadAndSwitchState(new ChartingState());
	}
