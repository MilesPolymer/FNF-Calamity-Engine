...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'pico-player': //I HAVE TO SWAP THE LEFT AND RIGHT ANIMATIONS BECAUSE FUCK FLIPX
				tex = Paths.getSparrowAtlas('characters/pico/Pico_PLAYABLE');
				iconColor = 0xff00ff0d;
				frames = tex;
				quickAnimAdd('idle', 'PICO idle dance');
				quickAnimAdd('singUP', 'PICO NOTE UP0');
				quickAnimAdd('singLEFT', 'PICO NOTE RIGHT0');
				quickAnimAdd('singRIGHT', 'PICO NOTE LEFT0');
				quickAnimAdd('singDOWN', 'PICO NOTE DOWN0');
				quickAnimAdd('singUPmiss', 'PICO NOTE UP MISS');
				quickAnimAdd('singLEFTmiss', 'PICO NOTE RIGHT MISS');
				quickAnimAdd('singRIGHTmiss', 'PICO NOTE LEFT MISS');
				quickAnimAdd('singDOWNmiss', 'PICO NOTE DOWN MISS');
				quickAnimAdd('hey', 'PICO HEY');

				animation.addByPrefix('scared', 'PICO idle shaking', 24, true);
	...
	{
		var offsets:Array<String> = CoolUtil.coolTextFile(Paths.file('data/character-data/offsets/' + char + '-data.json', TEXT, null));
		for (i in offsets)
		{
			var split = i.split(' ');
