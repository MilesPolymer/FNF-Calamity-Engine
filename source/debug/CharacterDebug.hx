...
class CharacterDebug extends MusicBeatState
{
	...
	override function create()
	{
		...
		background = new FlxSprite(-600, -525).loadGraphic(Paths.loadImage('stage_assets/week1/stageback', 'shared'));
		front = new FlxSprite(-650, 400).loadGraphic(Paths.loadImage('stage_assets/week1/stagefront', 'shared'));
		curt = new FlxSprite(-500, -300).loadGraphic(Paths.loadImage('stage_assets/week1/stagecurtains', 'shared'));
	...
	function saveBoyOffsets():Void
	{
		...
		if ((result != null) && (result.length > 0))
		{
			...
			_file.save(result.trim(), daAnim + "-data.json");
		}
	}
