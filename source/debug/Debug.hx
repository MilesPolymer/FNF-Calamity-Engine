...
class Debug
{
	...
	public static function onInitProgram()
	{
		...
		logInfo('Friday Night Funkin\' version: 0.2.8.0');
	...
	static function defineTrackerProfiles()
	{
		...
		FlxG.debugger.addTrackerProfile(new TrackerProfile(game.Character, ["curCharacter", "isPlayer", "barColor"], [FlxSprite]));
		FlxG.debugger.addTrackerProfile(new TrackerProfile(ui.HealthIcon, ["char", "isPlayer", "isOldIcon"], [FlxSprite]));
		FlxG.debugger.addTrackerProfile(new TrackerProfile(game.Note, ["x", "y", "strumTime", "mustPress", "rawNoteData", "sustainLength"], []));
		FlxG.debugger.addTrackerProfile(new TrackerProfile(game.Song, [
			"chartVersion",
			"song",
			"speed",
			"player1",
